...
export function generate(code, schema) {
  ...
  const allTypes = schema.types.map(function (schemaType) {
  ...
  traverse(ast, {
    TaggedTemplateExpression(path) {
      ...
      if (tagPath.node.name === "groq" && tagPath.referencesImport("groq")) {
        ...
        if (!type) {
          console.warn(`Issues generating groq query as no "_type" was found inside of a groq call.`);
          return;
        }
        ...
        Object.keys(rest).forEach(function(key) {
          ...
          if (!sanityDocument) {
            console.warn(`Issues generating type as no "_type" was found for ${key}.`);
            return;
          }
        ...
        if (isArray) {
          baseExport = t.exportNamedDeclaration(
            t.tSTypeAliasDeclaration(
              ...
              t.tSTypeReference(
                t.identifier("Array"),
                t.tsTypeParameterInstantiation([t.tSTypeLiteral(types.map(createTypes))])
              )
            )
          );
        } else {
